...
#pragma resource "*.dfm"
TPestADEiUserChnls *PestADEiUserChnls;
//---------------------------------------------------------------------------
__fastcall TPestADEiUserChnls::TPestADEiUserChnls(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::FormCreate(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::resetPestParamGrid()
{
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::resetChnlNo_CBox()
{
	ChnlNo_CBox->Items->Clear();
	for (int i = 1; i <= MDMi->getN(); i++) ChnlNo_CBox->Items->Add(i);
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::setPestGridParam(int idPestParam, std::vector<double> vTCEMMT)
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::ParamGridClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::ParamGridTopLeftChanged(TObject *Sender)
{
  PestParam_CBox->Visible = false;
  ChnlNo_CBox->Visible = false;
  UseParam_CBox->Visible = false;
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::PestParam_CBoxChange(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::ChnlNo_CBoxChange(TObject *Sender)
{
  ParamGrid->Cells[ParamGrid->Col][ParamGrid->Row] = ChnlNo_CBox->Text;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::UseParam_CBoxChange(TObject *Sender)
{
  ParamGrid->Cells[ParamGrid->Col][ParamGrid->Row] = UseParam_CBox->Text;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::ParTrans_CBoxChange(TObject *Sender)
{
  ParamGrid->Cells[ParamGrid->Col][ParamGrid->Row] = ParTrans_CBox->Text;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::Import_ButtonClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::OK_ButtonClick(TObject *Sender)
{
  PestParam_CBox->Visible = false;
  ChnlNo_CBox->Visible = false;
  UseParam_CBox->Visible = false;
...
//---------------------------------------------------------------------------
vector< vector<double> > __fastcall TPestADEiUserChnls::getPestParams() const
{
  return m_PestParams;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::setPestParams(vector< vector<double> > newPestParams)
{
  m_PestParams = newPestParams;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::BackToSavedParams()
{
  PestParam_CBox->Visible = false;
  ChnlNo_CBox->Visible = false;
  UseParam_CBox->Visible = false;
  ParTrans_CBox->Visible = false;
  vector< vector<double> > const pestParamVector = PestADEiUserChnls->getPestParams();
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::Cancel_ButtonClick(TObject *Sender)
{
  BackToSavedParams();
  Hide();
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::FormClose(TObject *Sender, TCloseAction &Action)
{
  BackToSavedParams();
}
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::newPestTplFile()
{
  int const n = MDMi->getN(); // number of channels
  if (n < 1)
  {
  ...
  {
	vector< vector<double> > const pestUserChnlParams(getPestParams()); // a series (vector) of vTCEMMT values (Type, Channel No., Estimate (No=0/Yes=1), Minimum, Maximum, Transform)
	...
	{
	  for (int idParam = 1; idParam <= 3; idParam++) // for each parameter (except qt)
	  {
		...
		{
		  tplFile << MDMi->getChnlParam(idChnl, idParam) << endl;
		}
	  }
...
//---------------------------------------------------------------------------
void __fastcall TPestADEiUserChnls::newPestControlFile()
{
  ...
  if ((PestGeneral->checkTikRegOption() == false) // Tikhonov regularisation mode not selected...
	  || ((PestGeneral->checkTikRegOption() == true) && (PestGeneral->getTikRegType() == 1) // or Tikhonov regularisation with "preferred homogeneity" option...
		  && (NmPAR <= 1) && (NhPAR <= 1) && (NsPAR <= 1))) // but only single parameters to optimize
  {
	PESTMODE = "estimation";
  }
  ...
  else // PESTMODE == "regularisation"
  {
	...
	{
	  NPRIOR = (NmPAR*(NmPAR-1) + NhPAR*(NhPAR-1) + NsPAR*(NsPAR-1)) / 2; // number of prior informations for the "preferred homogeneity" regularization: m1=m2, m1=m3, ... etc.
	  if (NmPAR > 1) NOBSGP++;
	  if (NhPAR > 1) NOBSGP++;
	  if (NsPAR > 1) NOBSGP++;
	}
  ...
  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
  {
	if ((pestUserChnlParams[i][0] != 0)&&(pestUserChnlParams[i][2] == 1))
	{
	  ...
	  if (pestUserChnlParams[i][0] == -1) pstFile << "  " << MDMi->getQT();
	  else if (pestUserChnlParams[i][0] == 1) pstFile << "  " << MDMi->getChnlParam(idChnl, 1);
	  else if (pestUserChnlParams[i][0] == 2) pstFile << "  " << MDMi->getChnlParam(idChnl, 2);
	  else if (pestUserChnlParams[i][0] == 3) pstFile << "  " << MDMi->getChnlParam(idChnl, 3);
  ...
  else if ((PESTMODE == "regularisation")&&(PestGeneral->getTikRegType() == 2)) // preferred value
  {
	...
	if (NmPAR >= 1)
	{
	  ...
	  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
	  {
		...
		{
		  int idChnl = pestUserChnlParams[i][1];
		  double m_init = MDMi->getChnlParam(idChnl, idparam);
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
	...
	if (NhPAR >= 1)
	{
	  ...
	  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
	  {
		...
		{
		  int idChnl = pestUserChnlParams[i][1];
		  double h_init = MDMi->getChnlParam(idChnl, idparam);
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
	...
	if (NsPAR >= 1)
	{
	  ...
	  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
	  {
		...
		{
		  int idChnl = pestUserChnlParams[i][1];
		  double s_init = MDMi->getChnlParam(idChnl, idparam);
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
