...
#pragma resource "*.dfm"
TPestADEniUserChnls *PestADEniUserChnls;
//---------------------------------------------------------------------------
__fastcall TPestADEniUserChnls::TPestADEniUserChnls(TComponent* Owner)
	: TForm(Owner)
{
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::FormCreate(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::resetPestParamGrid()
{
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::resetChnlNo_CBox()
{
	ChnlNo_CBox->Items->Clear();
	for (int i = 1; i <= MDMed->getN(); i++) ChnlNo_CBox->Items->Add(i);
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::setPestGridParam(int idPestParam, std::vector<double> vTTUMMT)
{ //vTTUMMT: Type, Channel No., Use (No=0/Yes=1), Minimum, Maximum, Transform
  if (vTTUMMT[0] == -1)
  {
	ParamGrid->Cells[1][idPestParam] = "Source Concentr.";
	ParamGrid->Cells[2][idPestParam] = "All";
  }
  else if (vTTUMMT[0] == 0) ParamGrid->Cells[1][idPestParam] = "None";
  else if (vTTUMMT[0] == 1) ParamGrid->Cells[1][idPestParam] = "Flowrate contrib. ratio";
  else if (vTTUMMT[0] == 2) ParamGrid->Cells[1][idPestParam] = "T0";
  else if (vTTUMMT[0] == 3) ParamGrid->Cells[1][idPestParam] = "Pe";
  else if (vTTUMMT[0] == 4) ParamGrid->Cells[1][idPestParam] = "Gamma Coeff.";
  if (ParamGrid->Cells[1][idPestParam] != "Source Concentr.") ParamGrid->Cells[2][idPestParam] = vTTUMMT[1];
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::ParamGridClick(TObject *Sender)
{
  ...
  {
	if (ParamGrid->Cells[ParamGrid->Col-1][ParamGrid->Row] != "Source Concentr.")
	{
	  ChnlNo_CBox->Items->Clear();
	  for (int i = 1; i <= MDMed->getN(); i++)
	  {
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::ParamGridTopLeftChanged(TObject *Sender)
{
  PestParam_CBox->Visible = false;
  ChnlNo_CBox->Visible = false;
  UseParam_CBox->Visible = false;
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::PestParam_CBoxChange(TObject *Sender)
{
  ParamGrid->Cells[ParamGrid->Col][ParamGrid->Row] = PestParam_CBox->Text;
  if (PestParam_CBox->Text == "Source Concentr.")
  {
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::ChnlNo_CBoxChange(TObject *Sender)
{
  ParamGrid->Cells[ParamGrid->Col][ParamGrid->Row] = ChnlNo_CBox->Text;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::UseParam_CBoxChange(TObject *Sender)
{
  ParamGrid->Cells[ParamGrid->Col][ParamGrid->Row] = UseParam_CBox->Text;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::ParTrans_CBoxChange(TObject *Sender)
{
  ParamGrid->Cells[ParamGrid->Col][ParamGrid->Row] = ParTrans_CBox->Text;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::Import_ButtonClick(TObject *Sender)
{
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::OK_ButtonClick(TObject *Sender)
{
  PestParam_CBox->Visible = false;
  ChnlNo_CBox->Visible = false;
  UseParam_CBox->Visible = false;
  ...
  for (int row = 1; row < ParamGrid->RowCount; row++)
  {
	...
	else if (ParamGrid->Cells[1][row] == "Source Concentr.") vTTUMMT[0] = -1;
	else if (ParamGrid->Cells[1][row] == "Flowrate contrib. ratio") vTTUMMT[0] = 1;
	else if (ParamGrid->Cells[1][row] == "T0") vTTUMMT[0] = 2;
	else if (ParamGrid->Cells[1][row] == "Pe") vTTUMMT[0] = 3;
	else if (ParamGrid->Cells[1][row] == "Gamma Coeff.") vTTUMMT[0] = 4;
	//------------------------------ Channel No. --------------------------------
	if (ParamGrid->Cells[2][row] == "All") vTTUMMT[1] = 0; // (c0 parameter)
...
//---------------------------------------------------------------------------
vector< vector<double> > __fastcall TPestADEniUserChnls::getPestParams() const
{
  return m_PestParams;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::setPestParams(vector< vector<double> > newPestParams)
{
  m_PestParams = newPestParams;
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::BackToSavedParams()
{
  PestParam_CBox->Visible = false;
  ChnlNo_CBox->Visible = false;
  UseParam_CBox->Visible = false;
  ParTrans_CBox->Visible = false;
  vector< vector<double> > const pestParamVector = PestADEniUserChnls->getPestParams();
  if (pestParamVector.size()>0)
  {
	for (int row = 1; row < ParamGrid->RowCount; row++)
	{
	  ...
	  {
		ParamGrid->Cells[1][row] = "Source Concentr.";
		ParamGrid->Cells[2][row] = "All";
	  }
	  else if (pestParamVector[row-1][0] == 1)
	  {
		ParamGrid->Cells[1][row] = "Flowrate contrib. ratio";
	  }
	  ...
	  {
		ParamGrid->Cells[1][row] = "Gamma Coeff.";
	  }
	  if (ParamGrid->Cells[1][row] != "Source Concentr.")
	  {
		ParamGrid->Cells[2][row] = pestParamVector[row-1][1];
	  }
	  ParamGrid->Cells[3][row] = pestParamVector[row-1][2];
	  ParamGrid->Cells[4][row] = FloatToStrF(pestParamVector[row-1][3], ffExponent, 3, 2);
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::Cancel_ButtonClick(TObject *Sender)
{
  BackToSavedParams();
  Hide();
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::FormClose(TObject *Sender, TCloseAction &Action)
{
  BackToSavedParams();
}
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::newPestTplFile()
{
  int const n = MDMed->getN(); // number of channels
  if (n < 1)
  {
  ...
  else
  {
	...
	bool optFlag_c0 = false; // code indiquant si le paramètre c0 doit être inversé ou non
	...
	if (!optFlag_c0) // c0 value fixed by the user, no optimization required
	{
	...
	{
	  for (int idParam = 1; idParam <= 4; idParam++) // for each parameter (except c0)
	  {
		...
		for (unsigned int k = 0; k < pestUserChnlParams.size(); k++) // checking whether the pair [idChnl][idParam] is listed in the Pest parameter vector
		{
		  ...
		  {
			optFlag = true;
			if (idParam == 1) {tplFile << "#f" << idChnl << "                #" << endl;}
			else if (idParam == 2) {tplFile << "#h" << idChnl << "                #" << endl;}
			else if (idParam == 3) {tplFile << "#s" << idChnl << "                #" << endl;}
			else if (idParam == 4) {tplFile << "#g" << idChnl << "                #" << endl;}
		  }
		}
		if (!optFlag) // User-fixed parameter (no optimization)
		{
		  tplFile << MDMed->getChnlParam(idChnl, idParam) << endl;
		}
	  }
...
//---------------------------------------------------------------------------
void __fastcall TPestADEniUserChnls::newPestControlFile()
{
  ...
  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
  {
	if ((pestUserChnlParams[i][0] != 0)&&(pestUserChnlParams[i][2] == 1))
	{
	  ...
	  if (pestUserChnlParams[i][0] == -1) Nc0PAR = 1; // number of c0-parameter to estimate
	  else if (pestUserChnlParams[i][0] == 1) NfPAR++; // number of f-parameters to estimate
	  else if (pestUserChnlParams[i][0] == 2) NhPAR++; // number of h-parameters to estimate
	  else if (pestUserChnlParams[i][0] == 3) NsPAR++; // number of s-parameters to estimate
	  else if (pestUserChnlParams[i][0] == 4) NgPAR++; // number of g-parameters to estimate
	}
  }
  string PESTMODE;
  if ((PestGeneral->checkTikRegOption() == false) // Tikhonov regularisation mode not selected...
	  || ((PestGeneral->checkTikRegOption() == true) && (PestGeneral->getTikRegType() == 1) // or Tikhonov regularisation with "preferred homogeneity" option...
		  && (NfPAR <= 1) && (NhPAR <= 1) && (NsPAR <= 1) && (NgPAR <= 1))) // but only single parameters to optimize
  {
	PESTMODE = "estimation";
  }
  ...
  else // PESTMODE == "regularisation"
  {
	...
	{
	  NPRIOR = (NfPAR*(NfPAR-1) + NhPAR*(NhPAR-1) + NsPAR*(NsPAR-1) + NgPAR*(NgPAR-1)) / 2; // number of prior informations for the "preferred homogeneity" regularization: m1=m2, m1=m3, ... etc.
	  if (NfPAR > 1) NOBSGP++;
	  if (NhPAR > 1) NOBSGP++;
	  if (NsPAR > 1) NOBSGP++;
	  if (NgPAR > 1) NOBSGP++;
  ...
  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
  {
	if ((pestUserChnlParams[i][0] != 0)&&(pestUserChnlParams[i][2] == 1))
	{
	  ...
	  // PARNME
	  if (pestUserChnlParams[i][0] == -1) pstFile << "c0";
	  else if (pestUserChnlParams[i][0] == 1) pstFile << "f" << idChnl;
	  else if (pestUserChnlParams[i][0] == 2) pstFile << "h" << idChnl;
	  else if (pestUserChnlParams[i][0] == 3) pstFile << "s" << idChnl;
	  else if (pestUserChnlParams[i][0] == 4) pstFile << "g" << idChnl;
	  // PARTRANS
	  ...
	  if (pestUserChnlParams[i][0] == -1) pstFile << "  " << MDMed->getC0();
	  else if (pestUserChnlParams[i][0] == 1) pstFile << "  " << MDMed->getChnlParam(idChnl, 1);
	  else if (pestUserChnlParams[i][0] == 2) pstFile << "  " << MDMed->getChnlParam(idChnl, 2);
	  else if (pestUserChnlParams[i][0] == 3) pstFile << "  " << MDMed->getChnlParam(idChnl, 3);
	  else if (pestUserChnlParams[i][0] == 4) pstFile << "  " << MDMed->getChnlParam(idChnl, 4);
  ...
  if (PESTMODE == "regularisation")
  {
	if (PestGeneral->getTikRegType() == 1) // preferred homogeneity
	{
	  if (NfPAR > 1) pstFile << "regulF" << endl;
	  if (NhPAR > 1) pstFile << "regulH" << endl;
	  if (NsPAR > 1) pstFile << "regulS" << endl;
	  if (NgPAR > 1) pstFile << "regulG" << endl;
	}
	else // preferred value
	{
	  if (Nc0PAR > 0) pstFile << "regulC0" << endl;
	  if (NfPAR > 0) pstFile << "regulF" << endl;
	  if (NhPAR > 0) pstFile << "regulH" << endl;
	  if (NsPAR > 0) pstFile << "regulS" << endl;
	  if (NgPAR > 0) pstFile << "regulG" << endl;
	}
  }
  //============ Observation Data Section ============
  pstFile << "* observation data" << endl;
  // on va chercher les temps dans TCobs_Form->Vect_Simulations
  // et on interpole les valeurs de concentration "observées" correspondantes
	...
	if (NfPAR > 1)
	{
	  ...
	  for (int i = 1; i < NfPAR; i++)
	  {
		...
		for (int j = 1; j < NfPAR-i+1; j++)
		{
		  ...
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
		  if (pestUserChnlParams[k][5] == 1) pstFile << "log(f" << pestUserChnlParams[k][1] << ") - 1.0 * log(f" << pestUserChnlParams[kk][1] << ")";
		  else pstFile << "f" << pestUserChnlParams[k][1] << " - 1.0 * f" << pestUserChnlParams[kk][1];
	...
	if (NgPAR > 1)
	{
	  ...
	  for (int i = 1; i < NgPAR; i++)
	  {
		...
		for (int j = 1; j < NgPAR-i+1; j++)
		{
		  ...
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
		  if (pestUserChnlParams[k][5] == 1) pstFile << "log(g" << pestUserChnlParams[k][1] << ") - 1.0 * log(g" << pestUserChnlParams[kk][1] << ")";
		  else pstFile << "g" << pestUserChnlParams[k][1] << " - 1.0 * g" << pestUserChnlParams[kk][1];
  ...
  else if ((PESTMODE == "regularisation")&&(PestGeneral->getTikRegType() == 2)) // preferred value
  {
	...
	if (Nc0PAR == 1)
	{
	  ...
	  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
	  {
		if ((pestUserChnlParams[i][0] == idparam)&&(pestUserChnlParams[i][5] == 1)) logTrC0 = true;
	  }
	  pstFile << "pi" << noPi << "  1.0 * ";
	  noPi++;
	  if (logTrC0) pstFile << "log(c0) = " << log10(c0_init);
	...
	if (NfPAR >= 1)
	{
	  ...
	  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
	  {
		...
		{
		  int idChnl = pestUserChnlParams[i][1];
		  double f_init = MDMed->getChnlParam(idChnl, idparam);
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
		  if (pestUserChnlParams[i][5] == 1) pstFile << "log(f" << idChnl << ") = " << log10(f_init);
		  else pstFile << "f" << idChnl << " = " << f_init;
	...
	if (NhPAR >= 1)
	{
	  ...
	  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
	  {
		...
		{
		  int idChnl = pestUserChnlParams[i][1];
		  double h_init = MDMed->getChnlParam(idChnl, idparam);
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
	...
	if (NsPAR >= 1)
	{
	  ...
	  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
	  {
		...
		{
		  int idChnl = pestUserChnlParams[i][1];
		  double s_init = MDMed->getChnlParam(idChnl, idparam);
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
	...
	if (NgPAR >= 1)
	{
	  ...
	  for (unsigned int i = 0; i < pestUserChnlParams.size(); i++)
	  {
		...
		{
		  int idChnl = pestUserChnlParams[i][1];
		  double g_init = MDMed->getChnlParam(idChnl, idparam);
		  pstFile << "pi" << noPi << "  1.0 * ";
		  noPi++;
		  if (pestUserChnlParams[i][5] == 1) pstFile << "log(g" << idChnl << ") = " << log10(g_init);
		  else pstFile << "g" << idChnl << " = " << g_init;
